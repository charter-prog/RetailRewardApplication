package com.patil.springboot.Dao;
...
public class CustomerRewardDao {
	...
	public Collection<TransactionRewards> getAllRewards() {
		...
		transactionRewardsTotal.setRewardMonth("ThreeMonthsTotalReward");
	...
	private static List<Transaction> getTransaction(){
		...
		Transaction transactionMonth = new Transaction();
		for(int i=0;i<=30;i++) {
		transactionMonth.setAmount((int)(Math.random()*100));		
		transactionList.add(transactionMonth);
		}
